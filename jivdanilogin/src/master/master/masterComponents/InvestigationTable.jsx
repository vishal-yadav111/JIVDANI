...
const CATEGORY_ID = Categories.INVESTIGATIONS.catID;
...
const InvestigationTable = () => {
  ...
  const columns = [{ key: "name", label: "Investigation Name", width: "80%", editable: true }];

  const load = async () => {
    setLoading(true);
    try {
      const res = await MasterApi(CATEGORY_ID, 1, search, 50);
  ...
  const onSave = async () => {
    const ok = await AddEditAll({ categoryId: CATEGORY_ID, name: editItem.name, id: editingId, extra: editItem });
    if (ok) {
      alert("Saved");
      setEditingId(null);
      load();
  ...
  return (
    <div>
      <div className="d-flex justify-content-between align-items-center mb-2">
        ...
        <div>
          <button className="btn btn-outline-primary me-2" onClick={() => downloadCSV(items, "investigation.csv")}>Export</button>
